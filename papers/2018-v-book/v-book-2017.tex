...
%NLG systems can be thought of a small and imperfect models of human language use, and as such they have the ability to make questions about language very tangible, so let us take the perspective of an NLG system for a moment. In a nutshell, NLG systems transform data and formulas into language \cite<e.g.,>[]{reiter2000building}, making decisions between different formulations of the same information. For example, if the temperature is 27.2 degrees Celsius, the system could express this as ``27.2 degrees'', ``approximately 27 degrees', ``above 25 degrees", or ``warm'', and the system must decide between these.  The problem is important in the generation of textual weather reports from numerical weather data such as temperature and wind speed \cite{goldberg1994using,turner2006generating}, and medical decision support on the basis of clinical measurements % such as oxygen saturation, heart rhythm, etc.
%\cite{Hripcsak01032009, hunter2008summarising, portet2009automatic}. Such systems often make decisions concerning the level of precision in the utterances that they generate on the basis of little more than intuition. Even when NLG systems are designed to mimic human language use \cite<e.g.,>{konstas2013global} there is no guarantee that these decisions taken by these systems benefit readers. A better understanding of the benefits (for readers and hearers) of different precision levels would allow these systems to become more useful. %The present article investigates the benefits, or de-benefits, of vagueness.
...
% KvD Feb 2017 (to address a criticism that came up in the reviews for the special issue):
...
Although our own studies in this article have focussed on vagueness in descriptive Noun Phrases, it seems plausible that vagueness plays a similar role in other linguistic constructs. For example, consider reports on air temperature. Given a numerical temperature measurement or prediction, we might word it as 
%
\begin{quote}
