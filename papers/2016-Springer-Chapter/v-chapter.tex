...
Vagueness pervades the language that we use on a daily basis. In everyday use, language use may be called vague for various reasons -- see e.g. the entry for \emph{vague} in \citet{penguin}.  In most academic use though, the word \emph{vagueness} has a more particular meaning. \citeauthor{keefe1997vagueness}, for example, state \begin{quotation}vague predicates have borderline cases\index{borderline cases}, have fuzzy boundaries, and are susceptible to sorites paradoxes \citep[p.\ 4]{keefe1997vagueness}\end{quotation} (a similar definition can be found in \citet{EgreKlinedinst}, among many others).  The most crucial of these criteria is the existence of borderline cases\index{borderline cases}: \begin{quotation}a word is precise if it describes a well-defined set of objects. By contrast, a word is vague if it is not precise \citep[p.\ 1]{lipmanvague}\end{quotation} A typical example is the word \emph{tall}, as applied to people for example, because here is no precise, known height which separates those who are tall from those who are not. The crucial point is that \emph{tall} admits borderline cases\index{borderline cases} (i.e., people who may or may not count as tall), which are the hallmark of vagueness as we use the term.
...
\label{accann}
Error rate data were analysed using a generalized logit mixed model \citep{jaeger2008categorical}, with vagueness and gap size and their interaction as independent variables, and with random slopes for vagueness and gap size over participants. 
%
