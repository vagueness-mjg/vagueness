...
\begin{APAenumerate}
	\item [(H1)] The main effect of \emph{vagueness} was to slow responses down, in contrast with Experiment 1, and offering evidence against hypothesis 1 (vague: 2668 ms; crisp: 2450 ms; a difference of 218 ms; $\beta=.06$, $se=.01$, $t=4.6$, $p<0.0001$) % MG added the values for these stats on 26 Sept 2016
	\item [(H2)] In focussed comparisons, vagueness was significantly disadvantageous in the \emph{numeric} conditions ($\beta=0.09$, $se=0.02$, $t=4.5$, $p<0.0001$)% MG added the values for these stats on 26 Sept 2016
				, and non-significantly disadvantageous in the \emph{verbal} conditions ($\beta=0.02$, $se=0.02$, $t=1.5$, $p=0.1550$)% MG added the values for these stats on 26 Sept 2016
				, offering partial evidence against hypothesis 2. The disadvantage for vagueness was greater in the numerical than in the verbal conditions, leading to a significant overall interaction effect between vagueness and instruction format ($\beta=-0.13$, $se=0.02$, $t=-6.6$, $p=0.0169$) % MG added the values for these stats on 26 Sept 2016
	\item [(H3)] There was a significant effect of \emph{instruction format} with numerical conditions attracting longer responses than the verbal conditions: consistent with Experiment 1 (numeric: 3284 ms; verbal 1866 ms; a difference of 1418 ms; ($\beta=-0.36$, $se=0.07$, $t=-5.1$, $p<0.0001$) % MG added the values for these stats on 26 Sept 2016
	\item [(H4)] There was a significant main effect of item (i.e., of which triple of numbers of dots were used in the stimulus): ($\beta=0.12$, $se=0.02$, $t=7.1$, $p<0.0001$). % MG added the values for these stats on 26 Sept 2016
				This effect seems likely to be due to the very fast responses for stimuli using arrangements of the smallest numbers of dots (6,15,24), which also had the largest difference in ratio of smallest number to largest number in the stimulus, suggesting that stimuli using this arrangement of numbers of dots may have been particularly discriminable for participants.
\end{APAenumerate}

\subsubsection{Borderline responses} % E2
A generalized linear mixed model \cite{jaeger2008categorical} was fit to the data for selection of the borderline response, with sum-coded vagueness, instruction format, (and their interaction), and item as fixed effects, and the same effects as slopes over participant for random effects.
...
\begin{APAenumerate}
	\item [(H5)] Participants were significantly more likely to choose the borderline option for vague instructions than for crisp instructions (21.9\% vs 11.3\%: $\beta=0.62$, $se=0.22$, $z=2.8$, $p=0.0059$). % MG added the values for these stats on 26 Sept 2016
	Participants were also significantly more likely to choose the borderline square when the instruction used the numerical format rather than the verbal format (30.1\% vs 3.0\%: $\beta=-3.35$, $se=0.23$, $z=-14.6$, $p<0.0001$). % MG added the values for these stats on 26 Sept 2016
\end{APAenumerate}
...
\textcolor{red}{insert citation supporting this}
...
RTs were trimmed at 2.5 sd separately for each subject, leading to the loss of 204 trials (2.8\% of the trials).
...
% E3
\begin{APAenumerate}
	\item [(H1)] Vague instructions were non-significantly easier for the reader than crisp instructions when vagueness was considered as a main effect ($\beta=-0.01$, $se=0.01$, $t=-0.4$, $p=0.7158$). % MG added the values for these stats on 26 Sept 2016
	\item [(H2)] There was a statistically significant main effect of selection task, with the comparison task speeding responses compared to the matching task ($\beta=0.16$, $se=0.03$, $t=6.2$, $p<0.0001$). % MG added the values for these stats on 26 Sept 2016
	\item [(H3)] Vagueness exerted effects in different directions for the comparison task and for the matching task: there was a significant interaction between vagueness and selection task ($\beta=0.13$, $se=0.03$, $t=4.2$, $p<0.0001$). % MG added the values for these stats on 26 Sept 2016
%				
Separate analyses were conducted testing for effects of vagueness at each level of the selection task, using Bonferroni significance thresholds to correct for multiple comparisons. 
Within the comparison task vagueness significantly speeded response times compared with crisp controls ($\beta=-0.07$, $se=0.02$, $t=-3.5$, $p<0.0012$). % MG added the values for these stats on 26 Sept 2016
Within the matching task vagueness significantly \emph{slowed} response times compared with crisp controls ($\beta=0.06$, $se=0.02$, $t=2.9$, $p<0.0061$). % MG added the values for these stats on 26 Sept 2016
\end{APAenumerate}
...
\begin{APAenumerate}
	\item [(H1)] There was no significant main effect of vagueness ($\beta=-0.02$, $se=0.01$, $t=-1.5$, $p=0.1296$). % MG added the values for these stats on 27 Sept 2016
	\item [(H2)] There was a main effect of selection, with comparison task instructions leading to faster responses than the matching task instructions ($\beta=0.18$, $se=0.02$, $t=10.4$, $p<0.0001$). % MG added the values for these stats on 27 Sept 2016
	This effect was in the same direction as Experiment 3. 
	\item[(H3)] Vagueness did exert different effects depending on the selection task (main interaction effect of vagueness by selection $\beta=0.12$, $se=0.02$, $t=5.1$, $p<0.0001$). % MG added the values for these stats on 27 Sept 2016
...
In the comparison task, vagueness resulted in faster response times ($\beta=-0.08$, $se=0.02$, $t=-4.3$, $p<0.0001$). % MG added the values for these stats on 27 Sept 2016
In the matching task vagueness slowed response times ($\beta=0.05$, $se=0.01$, $t=3.7$, $p=0.0004$). % MG added the values for these stats on 27 Sept 2016
...
\begin{table}[htbp]
\centering
\caption{Experiment 4: Instructions and targets}
\label{Instructions for e4}
\begin{tabular}{llll}
vagueness	&	instruction		&	selection							&	instruction: Choose a square with \ldots\\
			&	format		&	task		&			\\
\toprule
\multirow{ 2}{*}{crisp} 	 	& \multirow{ 2}{*}{verbal} 	&	matching 		& \ldots the same number of dots as the target \\ 
	& 	&	comparison										& \ldots fewer dots than the target \\
	\midrule
\multirow{ 2}{*}{vague} 	& \multirow{ 2}{*}{verbal} 	&	matching 			& \ldots about the same number of dots as the target \\ 
	& 	&	comparison										& \ldots far fewer dots than the target \\ 
\bottomrule
\end{tabular}
\end{table}%