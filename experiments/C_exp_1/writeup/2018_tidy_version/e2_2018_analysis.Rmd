...
```{r}
ggplot(data=raw_data, aes(RT_log)) + geom_density() +xlim(6,11)+ylim(0,0.8)
```
...
```{r}
ggplot(data=trim_data, aes(RT_log)) + geom_density()+xlim(6,11)+ylim(0,0.8)
```

```{r}
data.frame(obs=1:7680, raw=raw_data$RT_log, trim=c(trim_data$RT_log,rep(NA, times=239))) %>% 
  gather(key="type", value="value", raw, trim) %>%
  ggplot(aes(x=value, col=type)) + geom_density() + theme_bw() + theme(panel.grid = element_blank()) 
```
