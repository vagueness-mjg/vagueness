...
```{r lmer on logrt, warning=FALSE, cache=TRUE}
lmer2.log <- lmer(data=dat, rt_log ~ vag * subit + quan + gap + side + (1+vag|subject) + (1+vag|item))
dwplot(lmer2.log %>% tidy() %>% filter(group=="fixed")) + ggtitle("rt_log ~ vag * subit + quan + gap + side + (1+vag|subject) + (1+vag|item)")

lmer2.rcp <- lmer(data=dat, rt_reciprocal~subit*vag + (vag|subject) + (vag|item))

# lmer3 is closer to the original anova than lmer2
lmer3 <- tidy(lmer(data=dat, rt_reciprocal~vag*subit+quan+gap + (vag+subit|subject) + (vag|item)))
