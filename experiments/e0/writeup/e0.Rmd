...
if( !file.exists('data.txt') ) {
	system("cat ../experimentCode/output/*.dat > data.txt", wait=TRUE) # concatenate the individual subject data files into one file 
} 
```

```{r}
# # This bit is for generating the PURE file
...
write.table(dd, file='pilot_experiment_data.txt', col.names=T, row.names=F, quote=F, sep='\t')
```

```{r}
# This bit is for carrying forward
d <- read.table("data.txt", header=FALSE) # read in the data for all subjects
