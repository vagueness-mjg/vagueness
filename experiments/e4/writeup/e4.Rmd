...
Get data from the raw per-subject files.

```{r, getData}
#if( file.exists('dd.Rda') ) load('dd.Rda') else {
#system(wait=TRUE, paste("head -n 1 ", "../experimentCode","/output/", "subject01.data ", "> ", "../data/data.txt", sep=""))
#system(wait=TRUE, paste("tail -q -n +2 ", "../experimentCode", "/output/", "*.data ",  ">> ", "../data/data.txt", sep=""))
dat <- read.delim(file="../data/data.txt", header=T, stringsAsFactors=FALSE)
...
ggplot(temp, aes(score, colour=RTtype)) + 
  geom_freqpoly() + 
  facet_wrap(dfSubset~transformation, scales="free", nrow=2) +
...
n=length(y)
plot(y, xaxt="n", xlab="", ylab="RT_log\n", pch=19, ylim=extendrange(y, f=.10), main="Coefficients")
abline(h=0)
...
print(xtable(
  digits=c(0,2,2,1,1,3), 
  rtm.summary$coefficients), type='latex')
```
