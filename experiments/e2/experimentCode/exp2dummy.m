...
%         'Use the keys C,B, or M to choose the square that you think matches the instruction best.'));
    ...
    screenshotForInstructions=imread('screenshotA312.jpg', 'jpg');
...
%    Screen(windowPtr,'PutImage',imageArray,[rect],[copyMode])
    ...
    Screen('Flip', w);
	    ...
	    if takescreenshot==1
	       ...
	       imwrite(imageArray, 'screenshot_instructions.jpg');
	    end
...
for iterateBlocks = 1:numberOfBlocks
    ...
    for trial = 1:numberOfTrials
		...
		if takescreenshot==1
	       imageArray = Screen('GetImage', w, [0 0 1280 800]);
	       imwrite(imageArray, 'screenshot_trialMessage.jpg');
	    end
		...
		if takescreenshot==1
	       imageArray = Screen('GetImage', w, [0 0 1280 800]);
	       imwrite(imageArray, 'screenshot_dots.jpg');
	    end
    ...
    e=the_error = lasterror();
    the_error_message = the_error.message
    the_error_line = the_error.stack.line
    the_error_column = the_error.stack.column
