function exp3screenshots(SUBJECT)
...
%         'Use the keys C,B, or M to choose the square that you think matches the instruction best.'));
    ...
    screenshotForInstructions=imread('screenshotA312.bmp', 'jpg');
...
%    Screen(windowPtr,'PutImage',imageArray,[rect],[copyMode])
    ...
    if takescreenshot == 1
	 ...
	 imwrite(imageArray, 'screenshots/screenshot1_introductory_instructions.bmp');
	end
    ...
    if takescreenshot == 1
     imageArray = Screen('GetImage', w, [0 0 1280 800]);
	 imwrite(imageArray, 'screenshots/screenshot2_greyscreen.bmp');
	end    
    ...
    for trial = 1:numberOfTrials
	    ...
	    if takescreenshot==1
	       imageArray = Screen('GetImage', w, [0 0 1280 800]);
	       imwrite(imageArray, 'screenshots/screenshot3_practice_TrialMessage.bmp');
	    end
	    ...
	    if takescreenshot==1
	       imageArray = Screen('GetImage', w, [0 0 1280 800]);
	       imwrite(imageArray, 'screenshots/screenshot4_practice_responseScreen.bmp');
	    end
...
fclose(outputFilePointerForPracticeTrials)
    ...
    Screen('Flip',w);
	    if takescreenshot==1
	       imageArray = Screen('GetImage', w, [0 0 1280 800]);
	       imwrite(imageArray, 'screenshots/screenshot5_handover.bmp');
	    end
...
for iterateBlocks = 1:numberOfBlocks
    ...
    for trial = 1:numberOfTrials
	    ...
	    if takescreenshot==1
	       imageArray = Screen('GetImage', w, [0 0 1280 800]);
	       imwrite(imageArray, 'screenshots/screenshot6_TrialMessage.bmp');
	    end
	    ...
	    if takescreenshot==1
	       imageArray = Screen('GetImage', w, [0 0 1280 800]);
	       imwrite(imageArray, 'screenshots/screenshot7_TrialResponseScreen.bmp');
	    end
		    % Wait for a key  press
