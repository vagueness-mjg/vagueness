...
<<loadKnitr, echo=FALSE, message=FALSE>>=
library(knitr)
...
<<plotDists, fig.width=5, fig.height=5, fig.cap='Compare distributions of the various transformations of RT against random samples from normal distributions with the same mean and sd to see which transformations best approximate normal distributions', fig.pos='hbtp', fig.align='right', message=FALSE, warning=FALSE, echo=FALSE>>=
ggplot(temp, aes(time, colour=RTtype)) + 
  geom_freqpoly() + 
  facet_wrap(transformation~dfSubset, scales='free') + 
  scale_color_manual(values=c("blue","red")) + 
  theme_bw() + 
  theme(aspect.ratio=1, text=element_text(family='CMU Serif', size=12, face='italic'))
@

<<exampleOfFonts, fig.width=3, fig.height=3, fig.cap='font example', echo=FALSE>>=
...
<<boxPlots, fig.width=7, fig.height=3, fig.cap='Show how transformations of RT affect distribution of times, and how they affect which times are outliers.', echo=FALSE>>=
load('data_processed.Rda')
...
ggplot(subdata) + 
  ...
  xlab("") + ylab("")
  @
  <<fastSlowRT, fig.width=7, fig.height=3, fig.cap='Show how mean times for individual subjects and items vary with respect to the grand mean Log RT.', message=FALSE, echo=FALSE>>=
...
<<plotMainsa, fig.width=6, fig.height=6, fig.cap='Plot main effects on several transformations', message=FALSE, echo=FALSE, warning=FALSE>>=
...
<<mainB, fig.width=6, fig.height=3, fig.cap='Just the main effects', message=FALSE, echo=FALSE>>=
...
<<mainD, fig.width=6, fig.height=3.5, fig.cap='Main effects over item ratios', message=FALSE, echo=FALSE>>=
nrts <- cbind(Effect="Number",
xlev=factor(c(1,2)), 
myshape='a',
summarySEwithin(dd, measurevar="RT_log", withinvars=c("item_mean_ratio","Number")))
names(nrts)[5]<-"Levels"

orts <- cbind(Effect="Order",
xlev=factor(c(1,2)), 
myshape='b',
summarySEwithin(dd, measurevar="RT_log", withinvars=c("item_mean_ratio","Order")))
names(orts)[5]<-"Levels"

qrts <- cbind(Effect="Quantity",
xlev=factor(c(1,2)), 
myshape='d',
summarySEwithin(dd, measurevar="RT_log", withinvars=c("item_mean_ratio","Quantity")))
names(qrts)[5]<-"Levels"

vrts <- cbind(Effect="Vagueness",
xlev=factor(c(1,2)),
myshape='e',
summarySEwithin(dd, measurevar="RT_log", withinvars=c("item_mean_ratio","Vagueness")))
names(vrts)[5]<-"Levels"
...
p1=
ggplot(data=rts, aes(x=item_mean_ratio, y=RT_log, group=Levels)) +
...
breaks=c(0.5125000, 0.6876471, 0.7691558, 0.8166667),
labels=abbreviate(c(0.5125000, 0.6876471, 0.7691558, 0.8166667))) +
theme(aspect.ratio=1,
legend.position='top',
...
<<22way, fig.width=6, fig.height=3, fig.cap='2-way interactions over items', message=FALSE, echo=FALSE>>=
...
<<plotInt1, fig.width=7, fig.height=3.5, fig.cap='Plot vagueness by number interaction over items', echo=FALSE>>=
...
<<plot3way, fig.width=7, fig.height=5, fig.cap='3 way interactions',echo=FALSE>>=
...
<<vnitems, fig.width=7, fig.height=4, fig.cap='Plot vagueness by number by quantity over items.',echo=FALSE>>=
...
<<4way, fig.width=6, fig.height=3, fig.cap='4 way interaction', echo=FALSE>>=
...
<<4waysplit, fig.width=7, fig.height=7, fig.cap='4 way interaction split over items',echo=FALSE>>=
...
<<4waySplitRatio, fig.width=7, fig.height=7, fig.cap='4 way interaction split over items showing ratio of item dots.',echo=FALSE, message=FALSE, warning=FALSE>>=
...
<<showCompression, fig.width=7, fig.height=7, fig.cap='Show compression of item ratios', echo=FALSE>>=
...
plot(y=c(2,2,2,2), x=item_range_ratio, xlim=c(0,1), ylim=c(0,3), ylab="", yaxt='n', col="blue", pch=21, bg='blue', type='o', xaxt='n', xlab="ratio: small is easily discriminable", main="Discriminability of each item,\n expressed as ratios between array magnitudes")
...
<<plotLMERfnc, fig.width=7, fig.height=3.5, fig.cap='caption'>>=
par(mfrow=c(3,4), family='cmodern')
plotLMER.fnc(v5)
@

Plot model coefficients and ci's

<<v54, fig.width=7, fig.height=4, fig.cap='Estimates plot'>>=
...
<<baayenPLots99, fig.width=9, fig.height=3, fig.cap='caption'>>=
