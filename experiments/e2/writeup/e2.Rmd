...
library(Matrix); library(knitr); library(ggplot2); library(xtable); library(gridExtra); library(lme4); library(lmerTest);  library(data.table); library(dplyr); library(dtplyr); library(pander); library(LMERConvenienceFunctions); source('summarySEwithin2.R'); source('e2-preprocessing.R')
```
...
if( !file.exists('e2-b-data.txt') ) {
  ...
  write.table(dat, file='e2-b-data.txt', sep='\t', quote=FALSE, row.names = FALSE)
}
```
...
```{r getData, echo=T}
rawdata <- read.delim('e2-b-data.txt')
tempdata <- perSubjectTrim.fnc(rawdata[complete.cases(rawdata),], response='RT', subject='Subject', trim = 2.5)$data
rtdata <- tempdata
bldata <- tempdata
```
