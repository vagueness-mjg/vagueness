...
```{r, 'Concatenate the raw data'}
dat <- concatenate_raw_data()
dat %>% select(RT, RESPONSE, response_category) %>% summary() 
```

Catch impossible RTs: sticky fingers (RT<=1) and near timeouts (RT>59,998ms)
```{r, 'declare_impossible_rt'}
dat <- declare_impossible_rt(dat)
dat %>% select(RT, RESPONSE, response_category, NaType) %>% summary()
# show the NA trials
dat %>% filter(is.na(RT)) %>% select(RT_original, RESPONSE, NaType, RT, response_category) 
```

```{r, 'write the raw data to file'}
write.table(dat %>% select(Subject, Trial, Item, c_Itm, discriminability, Instruction, nchar_instr, Vagueness, Number, Order, Quantity, c_Vag, c_Num, c_Ord, c_Qty, NaType, response_category, isBorderline, RT, RT_log), file='e2-data.txt', sep='\t', quote=FALSE, row.names = FALSE)
```

Get the raw data.
```{r, 'get the raw data from file'}
...
```{r, 'select the response time data'}
rtdata <- trim_data
```

```{r, 'plot the distribution of the trimmed data', fig.width=3, fig.height=3}
ggplot(data=rtdata, aes(RT_log)) + geom_density() + theme(aspect.ratio = 1)
```
