...
## ----prepplotsMainEffects, message=FALSE, echo=FALSE, warning=FALSE------
# plot main effects on observed data
# allow reference to transformation
# raw
tempraw <- reshape2::melt(dd,
                          id.vars=c("Item", "Vagueness", "Number", "Quantity", "Order"),
                          measure.vars=c("RT_raw"),
                          variable.name="transformation",
                          value.name="score")
# allow reference to effect
tempraw2 <- reshape2::melt(tempraw,
                           id.vars=c("transformation", "score"),
                           measure.vars=c("Item", "Vagueness", "Number", "Quantity", "Order"),
                           variable.name="effect",
                           value.name="level")
# log
templog <- reshape2::melt(dd,
                          id.vars=c("Item", "Vagueness", "Number", "Quantity", "Order"),
                          measure.vars=c("RT_log"),
                          variable.name="transformation",
                          value.name="score")
# allow reference to effect
templog2 <- reshape2::melt(templog,
                           id.vars=c("transformation", "score"),
                           measure.vars=c("Item", "Vagueness", "Number", "Quantity", "Order"),
                           variable.name="effect",
                           value.name="level")

## ----plotMainEffectsRaw, fig.width=6, fig.height=3, fig.cap='Plot main effects in raw RT', fig.pos='hbtp', echo=FALSE----
# raw
ggplot(tempraw2, aes(y=score, x=level, group=1, col=effect)) +
  facet_wrap(~effect, scales="free", ncol=5) +
...
## ----plotMainEffectsLog, fig.width=6, fig.height=3, fig.cap='Plot main effects in log RT', fig.pos='htbp', echo=FALSE----
# log
ggplot(templog2, aes(y=score, x=level, group=1, col=effect)) +
  facet_wrap(~effect, scales="free", ncol=5) +
  stat_summary(fun.data=mean_cl_normal, geom="pointrange") +
  stat_summary(fun.y=mean, geom="line") +
  theme_bw() +
...
## ----mainD, fig.width=6, fig.height=5, fig.cap='Main effects in log RT over discriminability', fig.pos='hbtp', message=FALSE, echo=FALSE----
...
p1=
  ...
  xlab("Item discriminability measure")
p1

## ----22way, fig.width=7, fig.height=7, fig.cap='2-way interactions over discriminability', fig.pos='hbtp', message=FALSE, echo=FALSE----
...
## ----plotInt2, fig.width=6, fig.height=4, fig.cap='vagueness by number interaction over discriminability', fig.pos='hbtp', message=FALSE, echo=FALSE----
...
p1=ggplot(rts, aes(y=RT_log, x=Number, group=Vagueness, ymin=mins, ymax=maxs, pch=Vagueness)) +
  ...
  theme_bw() +
  theme(aspect.ratio=1) +
  theme(legend.title=element_blank(), legend.position="top", plot.background=element_rect(fill=NA, color='white'))
...
## ----vnitems, fig.width=6, fig.height=4, fig.pos='hbtp', fig.cap='Vagueness by number by quantity over discriminability',message = FALSE, echo=FALSE----
...
## ----4waysplit, fig.width=7, fig.height=7, fig.cap='4-Way interaction split over discriminability', fig.pos='hbtp', message=FALSE, echo=FALSE----
...
## ----plotModelCoefsAndCis, fig.width=5.5, fig.height=4, fig.cap='Coefficient estimates and their (Wald) 95 per cent confidence intervals', fig.pos='hbtp', echo=FALSE----
...
## ----lmerTestVersionOfModel, tidy=FALSE, cache=TRUE----------------------
...
## ----showgFunction, tidy=TRUE, size='scriptsize', echo=TRUE--------------
gatherData = function(number_of_valid_subjects) {
  data_dir <- '../experimentCode/output/'
  column.headers.df <-  head( read.table(
    paste(data_dir,'subject01.data',sep=''),
...
## ----showcFunction, tidy=TRUE, size='scriptsize', echo=TRUE--------------
classifyResponses = function(dat) {
  # what were they expected to respond?
  dat$crossed = as.factor(paste('Con', dat$Condition, ':Quan', dat$Quantity, ':Item', dat$Item, sep=''))
  dat[dat$crossed=='Con1:Quan1:Item1', 'Exp_Num']       <-  6
...
## ----showpFunction, tidy=TRUE, size='scriptsize', echo=TRUE--------------
processData = function(dat) {
  # SUBJECT
  dat$Subject=factor(paste("s",sprintf("%02d",dat$Subject),sep=""))
  # TRIAL
...
## ----showppFunction, tidy=TRUE, size='scriptsize', echo=TRUE-------------
postProcessData = function(dat) {
  # dd removes impossible trials from dat
  # Throw out RT = 1 and RT = 59998, and RTprev = 1 and RTprev = 59998 i.e., throw out sticky fingers and timeouts, and the trials that followed sticky fingers and timeouts since they were likely affected by unusual previous trials. Also lose impossible trials
  dd <- dat
