...
library(knitr); library(Matrix); library(ggplot2); library(xtable); library(gridExtra); library(lme4); library(lmerTest);  library(data.table); library(dplyr); library(dtplyr); library(pander); library(LMERConvenienceFunctions); source('summarySEwithin2.R'); source('e2-preprocessing.R')
```
...
```{r e2-blBarChart, fig.width=7, fig.height=3.5, echo=FALSE, message=FALSE, dev='pdf'}
...
```{r e2-rtlmerTest1Visuals, cache=TRUE, echo=FALSE, results='asis', dependson='e2-rtlmerTest1', fig.width=6, fig.height=3, dev='pdf'}
print(
  xtable(
...
```{r e2-bllmerVisuals, cache=TRUE, echo=FALSE, results='asis', dependson='e2-bllmer', fig.width=6, fig.height=3, dev='pdf'}
print(
  xtable(
